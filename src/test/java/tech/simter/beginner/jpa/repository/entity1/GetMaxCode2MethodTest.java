...
@DataJpaTest
class GetMaxCode2MethodTest {
  private Entity1JpaRepository repository;

  @Autowired
  GetMaxCode2MethodTest(Entity1JpaRepository repository) {
  ...
  void notFound() {
    // without data
    assertFalse(repository.getMaxCode2(codePrefix).isPresent());
  ...
  void found() {
    ...
    // verify
    Optional<String> maxCodeOptional = repository.getMaxCode2(codePrefix);
